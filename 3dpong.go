...
func NewGame() *Game {
	...
	c := &Game{
		...
		Arena:              f64.Vec3{X_WIDTH, Y_HEIGHT, Z_DEPTH},
		PaddleSize:         f64.Vec2{PADDLE_WIDTH, PADDLE_HEIGHT},
	...
	for i := range c.Queue {
		c.Queue[i] = make([]f64.Vec2, QUEUE_SIZE)
		c.QueuePos[i] = 0
	}
	return c
}
...
func (c *Game) reset() {
	...
	for i := 0; i < 2; i++ {
		...
		c.View[i] = 0
		c.Score[i] = 0
		c.Angle[i] = f64.Vec2{5, 5}
		c.recalculateTrig(i)

		for j := range c.Queue {
			c.Queue[i][j] = f64.Vec2{}
		}
	}
...
func (c *Game) xmouse() int {
	...
	return int(f64.LinearRemap(float64(mx), float64(v.X), float64(ow)-float64(v.X), 0, float64(c.Width)))
}
...
func (c *Game) event(pln int, ev interface{}) bool {
	...
	case sdl.MouseButtonDownEvent:
		...
		c.OldPos[pln] = f64.Vec2{float64(ev.X), float64(ev.Y)}
	...
	case sdl.MouseMotionEvent:
		if c.OldButton[pln] == sdl.BUTTON_LEFT || c.NoClick[pln] {
			...
			c.Queue[pln][c.QueuePos[pln]] = f64.Vec2{float64(ev.Xrel), float64(ev.Yrel)}
			...
			c.Player[pln].X = f64.Clamp(c.Player[pln].X, -c.Arena.X+c.PaddleSize.X, c.Arena.X-c.PaddleSize.X)
			c.Player[pln].Y = f64.Clamp(c.Player[pln].Y, -c.Arena.Y+c.PaddleSize.Y, c.Arena.Y-c.PaddleSize.Y)

			c.OldPos[pln] = f64.Vec2{float64(ev.X), float64(ev.Y)}
		} else if c.OldButton[pln] == sdl.BUTTON_MIDDLE {
			c.Angle[pln] = c.Angle[pln].Add(f64.Vec2{float64(ev.Xrel), float64(ev.Yrel)})
			c.Angle[pln] = c.Angle[pln].Wrap(0, 360)
...
func (c *Game) moveCrap() {
	// Move crap
	c.CrapPos = c.CrapPos.Add(c.CrapVel)
	if c.CrapPos.X < -c.Arena.X {
		c.CrapPos.X = -c.Arena.X
		c.CrapVel = f64.Vec3{0, 0, 1}
...
func (c *Game) total(queue []f64.Vec2) f64.Vec2 {
	var v f64.Vec2
	for _, q := range queue {
		v.X += q.X
		v.Y += q.Y
...
func (c *Game) drawDebris(pln int) {
	for i := range c.Debris {
		...
		if d.Exist {
			...
			c.drawLine(pln, d.Pos, d.Pos.Add(d.Vel), col)
		}
	}
}
...
func (c *Game) drawLine(pln int, p1, p2 f64.Vec3, col color.RGBA) {
	re := c.Renderer
	...
	for i := 0; i < c.Glasses[pln]+1; i++ {
		...
		case 1: // From the side
			pp1 = f64.Vec3{p1.Z, p1.Y, p1.X}
			pp2 = f64.Vec3{p2.Z, p2.Y, p2.X}

		case 2: // From above
			pp1 = f64.Vec3{p1.X, p1.Z, p1.Y}
			pp2 = f64.Vec3{p2.X, p2.Z, p2.Y}
		...
		case 3: // Free view
			...
			pp1 = f64.Vec3{xx1, yy1, zz1}
			pp2 = f64.Vec3{xx2, yy2, zz2}

		case 4: // Watch the ball
			ball := &c.BallPos
			anglex := (ball.Z - c.Arena.Z) / 10
			anglex = f64.Clamp(anglex, -90, 90)
